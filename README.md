...
확인이 끝나야 AI 단추가 열린다. 저장해 둔 키가 있으면 페이지를 열 때마다 다시 확인한다.
...
구글이 모델 이름을 갈아 치우는 일은 예고 없이 일어난다. 이름 하나에 매이지 않도록 네 겹으로 둔다.

| 겹 | 하는 일 |
|---|---|
| ① 실행 시점 조회 | 모델 이름을 코드에 박지 않는다. 그때그때 목록을 받아 flash 계열 최신 안정판을 앞세운다 |
| ② 성공한 이름 기억 | 마지막에 응답한 모델을 기억해 다음번에 맨 앞에 세운다. 그 이름이 404면 즉시 버린다 |
| ③ 목록 창구가 막혀도 진행 | 목록 조회가 실패해도 멈추지 않고 내장 이름(`gemini-2.5-flash` 등)으로 밀어붙인다. 다만 「목록을 못 받았다」는 사실은 화면에 남긴다 |
| ④ 사람이 고르기 | [우선 모델]에서 살아 있는 목록을 직접 고를 수 있다. 고른 모델이 죽으면 [자동 모델로 되돌려 다시 확인]으로 빠져나온다 |

한도 초과(429)는 2초 쉬었다 다음 모델, 지원 중단(404)은 즉시 다음 모델, 키 오류는 즉시 중단한다.
